...
- Migraciones remotas: `20260829153000`, `20260829154000`, `20260829170000`, `20260829171000`, `20260829171100`, `20260829171200`, `20260829180000`, `20260829181000`, `20260829181100`, `20260829190000`, `20260829190100`, `20260829190200`, `20260829200000`, `20260829201000`, `20260829202000`, `20260829210000`, `20260829220000`, `20260830090000`, `20260830091000`, `20260830092000`, `20260830100000`, `20260830110000`, `20260830110100`, `20260830130000`, `20260830130100`, `20260830150000`, `20260830150100`, `20260830170000`, `20260902120000`, `20260902180000`, `20260902181000`, `20260902190000`
- Edge Functions con JWT de usuario: `sync-intelligence`, `satellite-analytics`, `satellite-timeseries`, `water-balance`, `agro-intelligence` y `scouting-evidence`. Funciones con token independiente de dispositivo: `ingest-telemetry` y `device-control`.
...
- Pilot Control iniciado antes de intervenir, con hipótesis y criterio firmados por el piloto; captura semanal, revisión de calidad y cierre final ensayados.
...
- Scout Field: bucket privado, metadata append-only, hash server-side, enlaces firmados, análisis estructural y consulta de hash en catálogos públicos. `scouting-evidence` está desplegada; `OPTIONS` respondió 200 y una carga sin sesión fue rechazada con 401. La sesión owner mostró `0 observaciones · 0 fotos` sin insertar evidencia ficticia.
...
- OAuth y PWA: Google usa PKCE y el callback se sanea después de recuperar la sesión. El build genera manifest, registro y service worker con 28 recursos estáticos; no existe runtime cache para Supabase, TUS ni imágenes privadas.
- NODO Field Offline v3: además de la cola binaria v2, la bóveda prepara un paquete Scout tenant-scoped de hasta 4 MB/24 horas, filtra asignaciones de operador y recupera una recarga sin servidor sin cachear respuestas. El modo reconstruido limita la navegación a Recorridas, evita mutaciones de agenda y mantiene captura cifrada; la sincronización procesa hallazgos antes que fotos y el servidor conserva autoridad final.
- Validación de Field Offline v3: la sesión `owner` activó una bóveda temporal sobre el workspace real con `0 recorridas abiertas`, generó el paquete y se bloqueó. Un service worker de prueba con alcance aislado interrumpió sólo Supabase; dos recargas limpias mostraron el acceso de contingencia, exigieron la frase y reconstruyeron Scout con preparación/vencimiento visibles y sin consultas de evidencia. La vista 390×844 y el retorno al servicio normal se verificaron; cero errores o advertencias posteriores a la carga limpia. El aislamiento, su registro y la bóveda temporal fueron eliminados, y no se insertaron filas ni objetos ficticios. La captura/reanudación TUS autenticada con una recorrida real continúa como gate del dispositivo piloto.
...
- El test pgTAP incluye contratos de identidad y operación dentro del total vigente. No se ejecutó en esta estación porque Docker Desktop no estaba activo; el build, el lint remoto, los endpoints y las lecturas autenticadas sí fueron verificados. CI debe ejecutar el pgTAP antes de integrar.
- Pilot Control: tablas, RLS, funciones de cálculo y state machine desplegadas. La sesión real `owner` consultó cero pilotos, mostró el formulario de lanzamiento y fue validada en escritorio y 390×844 sin crear una línea base ficticia. La consola no registró errores.
- NODO Earth Time: migración `20260902120000_earth_time.sql` aplicada en remoto; `satellite-timeseries`, `sync-intelligence` y `agro-intelligence` desplegadas. `OPTIONS` respondió 200 y una llamada sin sesión fue rechazada con 401. `supabase db lint --linked --level warning` no reportó errores. El contrato pgTAP total subió a 132 aserciones. La construcción autenticada de una serie sobre lotes reales queda como validación de campo; no se insertaron observaciones ficticias.
- NODO Water: migraciones `20260902180000_nodo_water.sql` y `20260902181000_fix_irrigation_request_id.sql` aplicadas; `water-balance`, `agro-intelligence` y `satellite-timeseries` desplegadas. `OPTIONS` respondió 200 y un POST sin sesión fue rechazado con 401. El lint remoto quedó limpio tras calificar `request_id` en las RPC. El contrato pgTAP total subió a 148 aserciones. No se declararon riegos ni saldos ficticios; el cálculo autenticado sobre lotes reales queda como validación de campo.
- NODO Scout Field: migración `20260902190000_scout_field_scan.sql` aplicada; `scouting-evidence` redesplegada. `OPTIONS` respondió 200 y un POST sin sesión fue rechazado con 401. El lint remoto quedó limpio. El contrato pgTAP total subió a 160 aserciones. No se subieron fotografías de prueba; el rechazo de un políglota real queda como validación de campo.
- NODO Terrain: migración `20260902200000_nodo_terrain.sql` aplicada; `terrain-relief` y `agro-intelligence` desplegadas. `OPTIONS` respondió 200 y un POST sin sesión fue rechazado con 401. El lint remoto quedó limpio. El contrato pgTAP total subió a 172 aserciones. No se inventaron cotas; el cálculo autenticado sobre lotes reales queda como validación de campo.
- NODO Outcome Ledger: migración `20260903010000_outcome_ledger.sql` aplicada. RPCs de ciclo, labor, costo, declaración y revisión activas. El contrato pgTAP total subió a 184 aserciones. No se inventaron montos; los resultados usan asientos del libro.
