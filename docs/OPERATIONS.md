...
- NODO Field Offline v2: la bóveda AES-GCM cifra por separado metadatos y bytes, verifica firma y SHA‑256, aplica límites de 8 MB/archivo, 12 fotos y 64 MB pendientes y conserva cifrada la URL TUS de reanudación. La sincronización procesa hallazgos antes que fotos y el servidor sólo finaliza una evidencia después de volver a descargar y verificar el objeto.
- Validación de Field Offline v2: la sesión `owner` activó una bóveda temporal y el self-test del navegador completó round-trip y rechazo de manipulación para JSON y 1 KB binario. La interfaz vacía se validó en escritorio y 390×844; la bóveda temporal quedó eliminada, el establecimiento conserva `0 observaciones · 0 fotos` y la consola no registró errores ni advertencias. `scouting-evidence` y la RPC endurecida están desplegadas; `OPTIONS` respondió 200 y una llamada sin sesión fue rechazada con 401. La prueba autenticada de captura, corte y reanudación TUS end-to-end queda pendiente de una recorrida real en curso y un dispositivo del piloto; no se crearon datos ficticios para simularla.
