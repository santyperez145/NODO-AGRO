...
El núcleo técnico del piloto ya cuenta con identidad real y OAuth PKCE, aislamiento multiempresa, onboarding sin datos simulados, clima persistido, NODO Earth con escenas Sentinel‑2 e índices reales por lote, NODO Earth Time con serie SCL de 90 días y mediana empírica por polígono, NODO Water con saldo de referencia FAO‑56 y riego declarado, NODO Terrain con Copernicus DEM GLO-30 (DSM ~30 m, LE90ABS y licencia visibles), NODO Outcome Ledger con ciclos señal→labor→costo→resultado verificable internamente, NODO Scout con verificaciones georreferenciadas, fotografía privada auditable y análisis de campo antes de adjuntar, PWA instalable, ingestión segura, plano de control IoT con gemelos, órdenes de trabajo de flota y una capa de inteligencia auditable desplegada. La cuenta de servicio de inteligencia está configurada, pero todavía necesita crédito API y evaluación sistemática. La teledetección orienta recorridas y no equivale a diagnóstico agronómico. El producto no equivale a validación comercial ni regulatoria: requiere dispositivos físicos calibrados, resultados del establecimiento piloto, pruebas de seguridad y los gates de `docs/LAUNCH_READINESS.md` antes de vender o actuar sobre el campo.
