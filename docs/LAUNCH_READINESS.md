...
Actualizado: 2026-08-30. Esta matriz es un instrumento de gestión; **no constituye asesoramiento legal ni otorga una homologación**. Cada obligación debe confirmarse para el SKU, contrato, flujo de datos, provincia y actividad concretos con profesionales y organismos competentes antes de vender.
...
|---|---|---|---|
| Privacidad y datos personales | Usuarios, responsables, técnicos, ubicación de recorridas, imágenes o identificadores vinculables a personas | RLS multiempresa, invitaciones ligadas al correo, baja por empresa, auditoría de acceso server-owned, ubicación opcional, OAuth PKCE, fotografías en bucket privado, hash server-side y URL temporal implementados. El caché PWA excluye datos sensibles; el paquete Scout de 24 horas, hallazgos, metadatos y bytes de fotos offline se conservan en una bóveda local AES-GCM desbloqueada por el usuario, con minimización por rol, cuota visible y persistencia del origen opt-in. Sólo el UUID de la última identidad queda en claro para contingencia y se elimina al cerrar sesión. Aviso de privacidad, inventario, retención, ejercicio de derechos, política de dispositivos, contratos con encargados y análisis de transferencias: pendientes | Mapa de tratamientos y subencargados; finalidad/base legal para correo, roles, último acceso, ubicación e imágenes de personal; aviso y términos revisados; proceso de acceso, rectificación y supresión; plazos de retención y borrado de objetos y borradores locales; evaluación de transferencias internacionales; política ante pérdida de frase/dispositivo; decisión documentada sobre inscripción de bases. La [AAIP exige informar finalidad, responsable y derechos, y aplicar seguridad y confidencialidad](https://www.argentina.gob.ar/aaip/datospersonales/responsables/obligaciones). |
...
| Teledetección y cartografía | Imágenes, índices, mapas térmicos, relieve y recomendaciones por lote | Sentinel‑2 fechado, NDVI/NDMI por polígono, serie SCL 90/180 días, campañas julio–junio, nubosidad SCL por lote, mediana empírica y límites explícitos implementados | Inventario de licencias y atribuciones; protocolo SCL y control de calidad; comparación contra recorridas y sensores; precisión por cultivo/zona; versionado de algoritmo; revisión de claims. GeoJSON y superficie estimada no sustituyen mensura ni catastro. |
| Agua e hidrología | ET0, lluvia, riego declarado, NDMI y humedad de suelo | Saldo de referencia FAO‑56, bitácora append-only, cobertura de evidencia y límites explícitos implementados | Kc/ETc, calibración de sonda, escorrentía, capacidad de campo, prescripción de lámina y control de bombas: pendientes y no se afirmarán. |
...
- SMTP propio y dominio de redirección no bloquean P0 interno si sólo se usan cuentas del equipo; sí bloquean toda invitación a pilotos externos.

### Gate P1 — Piloto en campo

- Contrato de piloto y anexo de datos firmados.
- Dominio definitivo, SMTP transaccional, SPF/DKIM/DMARC y flujo completo de invitación/offboarding ensayados con identidades reales.
...
- Bóveda offline v3 ensayada en el dispositivo de campo objetivo: preparación y vencimiento del paquete, sesión remota vencida, hallazgo y foto real, bloqueo, corte de red, reinicio, reconexión, pausa/reanudación TUS, rechazo por permiso/estado, sincronización idempotente, hash coincidente, cuota/evicción y procedimiento por frase perdida. Scout Field ya define el análisis de evidencia; este gate no se supera hasta registrar el ensayo físico. Archivos no controlados siguen prohibidos.
