...
export function OfflineVaultPanel({scope,onSyncDraft,onSyncMedia}:{scope:OfflineDraftScope;onSyncDraft:(draft:OfflineScoutingFindingDraft)=>Promise<string>;onSyncMedia:(draft:OfflineScoutingMediaDraft,file:File,onProgress:(percentage:number)=>void,onUploadUrl:(url:string|null)=>Promise<void>)=>Promise<unknown>}){
  ...
  async function synchronize(){
    if(!navigator.onLine){setError('Seguís sin conexión. Los hallazgos y fotos permanecen cifrados; no se descartó nada.');return}
  ...
  async function reset(){setBusy('reset');setError('');try{await resetOfflineVault(scope.userId);setDrafts([]);setMedia([]);setConfirmReset(false);setNotice('Bóveda restablecida. Hallazgos y fotos cifrados locales fueron eliminados.');setPassphrase('');setConfirmation('')}catch(cause){setError(errorMessage(cause))}finally{setBusy(null)}}
  ...
  return <section className={`offlineVault ${vault.status==='unlocked'?'unlocked':''}`}>
    ...
    {expanded&&<div className="offlineVaultBody">
      ...
      {vault.status==='unlocked'&&<>
        ...
        <div className="vaultReset">{confirmReset?<><p>Esto elimina todos los hallazgos y fotos cifrados del usuario en este dispositivo. No puede deshacerse.</p><button type="button" disabled={busy!==null} onClick={()=>void reset()}><Trash2/>{busy==='reset'?'Eliminando…':'Confirmar restablecimiento'}</button><button type="button" onClick={()=>setConfirmReset(false)}>Cancelar</button></>:<button type="button" onClick={()=>setConfirmReset(true)}><RotateCcw/>Restablecer bóveda local</button>}</div>
      </>}
