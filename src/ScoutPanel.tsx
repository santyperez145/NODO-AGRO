...
import { Camera, CheckCircle2, ClipboardCheck, FileImage, Flag, Images, LoaderCircle, LocateFixed, MapPin, Navigation, Plus, Route, ShieldAlert, ShieldCheck, XCircle } from 'lucide-react';
import {
...
export function ScoutPanel({data,seed,onSeedConsumed}:{data:Workspace;seed:ScoutSeed|null;onSeedConsumed:()=>void}){
  ...
  useEffect(()=>{setMediaSelection(null);setMediaCaption('');setMediaCapturedAt(localDateTime());setMediaValidation('')},[mediaFinding]);
  ...
  function submitMedia(event:FormEvent){
    ...
    uploadEvidence.mutate({findingId:mediaFinding,file:mediaSelection.file,caption:mediaCaption,capturedAt:new Date(mediaCapturedAt).toISOString(),captureSource:mediaSelection.source},{onSuccess:()=>setMediaFinding(null)});
  }
  ...
  return <section className="scoutModule">
    ...
    <div className="scoutList">{orderedVisits.length?orderedVisits.map(visit=>{
      ...
      return <article key={visit.id} className={`scoutCard priority-${visit.priority} ${overdueVisit?'overdue':''}`}>
        <div className="scoutIdentity"><span className={`scoutStatus ${visit.status}`}>{visitStatusLabels[visit.status]}</span><small>{priorityLabels[visit.priority]}{overdueVisit?' · vencida':''}</small><h3>{visit.title}</h3><p>{parcel?.name??'Lote histórico'} · {new Date(visit.scheduled_for).toLocaleString('es-AR')}</p></div>
