...
export type ScoutSeed={parcelId:string;metricId:string;indexName:SatelliteIndexName;meanValue:number;objective?:string};
...
function scanLabel(item:ScoutingFindingMedia){
  if(item.scan_verdict==='clean')return 'Hash consultado · sin coincidencia';
  if(item.scan_verdict==='unknown')return 'Análisis estructural · catálogo incompleto';
  if(item.scan_verdict==='blocked')return 'Rechazada por análisis de campo';
  return 'Sin análisis de campo persistido';
}
...
function EvidenceGallery({media,urls,offline}:{media:ScoutingFindingMedia[];urls:Record<string,string>;offline:boolean}){
  ...
  return <div className="evidenceGallery">{media.map(item=>{
    ...
    return <article key={item.id}>
      ...
      <div><b>{item.caption||item.original_filename}</b><span>{fileSize(item.size_bytes)} · {item.capture_source==='camera'?'cámara':'archivo'} · {new Date(item.captured_at).toLocaleString('es-AR')}</span><small>SHA‑256 {item.sha256.slice(0,12)}…</small><em className={`scanBadge ${item.scan_verdict??'pending'}`}>{scanLabel(item)}</em></div>
    </article>;
  })}</div>;
}
...
export function ScoutPanel({data,seed,onSeedConsumed,offlineMode=false}:{data:Workspace;seed:ScoutSeed|null;onSeedConsumed:()=>void;offlineMode?:boolean}){
  ...
  useEffect(()=>{
    ...
    setForm({parcelId:seed.parcelId,sourceMetricId:seed.metricId,title:`Verificar señal ${seed.indexName.toUpperCase()} · ${parcel?.name??'Lote'}`,objective:seed.objective??`Contrastar en campo la señal relativa ${seed.meanValue.toFixed(3)} de la escena satelital y registrar evidencia antes de decidir una intervención.`,priority:'high',scheduledFor:localDateTime(60),assigneeId:defaultAssigneeId});
    setCreating(true);
  ...
  function submitMedia(event:FormEvent){
    ...
    uploadEvidence.mutate({findingId:mediaFinding,file:mediaSelection.file,caption:mediaCaption,capturedAt:new Date(mediaCapturedAt).toISOString(),captureSource:mediaSelection.source,onProgress:setMediaUploadProgress},{onSuccess:()=>{setFieldNotice('Foto transferida por TUS, analizada en servidor y adjuntada al hallazgo.');setMediaFinding(null)}});
  }
