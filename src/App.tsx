...
import { Activity, Beef, Bot, CloudRain, Database, Droplets, Gauge, GitBranch, HardDrive, KeyRound, Leaf, LoaderCircle, LogOut, Map, Pencil, Radio, RefreshCw, Satellite, ScanSearch, ShieldAlert, ShieldCheck, Target, Tractor, TrendingUp, Users, Wifi, WifiOff } from 'lucide-react';
...
const sections = [[Gauge,'Centro de mando'],[Map,'Mapa vivo'],[Leaf,'Cultivos'],[Droplets,'Agua'],[ScanSearch,'Recorridas'],[Beef,'Rodeo'],[Tractor,'Maquinaria'],[Radio,'Sensores'],[TrendingUp,'Economía'],[GitBranch,'Resultados'],[Target,'Piloto'],[Users,'Equipo']] as const;
...
const OutcomePanel = lazy(() => import('./OutcomePanel').then(module => ({ default:module.OutcomePanel })));
...
function Overview({data,weather,evidenceScore,staleDevices,onDecision,onOpenResults}:{data:Workspace;weather:WeatherValue|null;evidenceScore:number;staleDevices:number;onDecision:(id:string,status:'accepted'|'dismissed')=>void;onOpenResults:()=>void}){
  const establishment=data.establishment!;
  const sceneAge=data.satellite?Math.floor((Date.now()-new Date(data.satellite.captured_at).getTime())/86_400_000):null;
  const verifiedAmount=Number(data.outcomeLedgerSummary?.internally_verified_amount??0);
  return <>
...
function Decision({item,onDecision,onOpenResults}:{item:Recommendation;onDecision:(id:string,status:'accepted'|'dismissed')=>void;onOpenResults:()=>void}){return <div className="decision real"><span className={`priority ${item.priority}`}>{item.priority}</span><div><small>{item.confidence}% CONFIANZA · {item.valid_until?`VÁLIDA HASTA ${new Date(item.valid_until).toLocaleString('es-AR')}`:'SIN VENCIMIENTO'}</small><h3>{item.title}</h3><p>{item.rationale}</p><b>{item.action}</b></div><div className="decisionActions"><button onClick={()=>onDecision(item.id,'accepted')}>Aceptar</button><button onClick={()=>onDecision(item.id,'dismissed')}>Descartar</button><button onClick={onOpenResults}>Cerrar en Resultados</button></div></div>}
