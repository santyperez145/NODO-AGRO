...
const promptVersion = 'operational-brief-v2';
...
Deno.serve(async request => {
  ...
  try {
    ...
    const [parcels, devices, readings, weather, satellite, satelliteMetrics, satelliteBaselines, waterBalances, irrigations, terrainMetrics, recommendations, scoutingVisits, scoutingFindings, livestock, machinery, workOrders, finances, operations] = await Promise.all([
      ...
      admin.from('satellite_scenes').select('captured_at,cloud_cover_pct,provider,collection').eq('establishment_id', establishmentId).order('captured_at', { ascending: false }).limit(12),
      admin.from('parcel_satellite_metrics').select('parcel_id,index_name,captured_at,mean_value,stddev_value,quality_status,pixel_count,algorithm_version,scl_cloud_percent').eq('establishment_id',establishmentId).order('captured_at',{ascending:false}).limit(400),
